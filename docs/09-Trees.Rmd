...
background-image: url(images/title_slide_trees.jpeg)
...
## [Brad's intro to random forests](https://bradleyboehmke.github.io/random-forest-training/slides-source.html#1)
...
# remotes::install_github("koalaverse/vip")  # for variable importance plots
```
...
knitr::include_graphics("images/edible.jpg")
```
...
path <- paste0("https://raw.githubusercontent.com/bgreenwell/",
               "MLDay18/master/docs/data/mushroom.csv")
...
Use the [`pROC`](https://cran.r-project.org/package=pROC) package to compute and plot the train and test ROC curves based on the shallow tree (i.e., `cart2`). What is AUC for the train and test sets?

**Hint:** Just modify the code from the job attrition example in .red[`08-SVM.R`].
...
```{r mushroom-cart-roc-01, eval=FALSE}
# Load required packages
library(pROC)

# Plot train and test ROC curves
roc_trn <- roc(  # train AUC: 0.9717
  predictor = predict(cart1, newdata = trn, type = "prob")[, 1L], 
  response = trn$Edibility,
  levels = rev(levels(trn$Edibility))
)
roc_tst <- roc(  # test AUC: 0.8567
  predictor = predict(cart1, newdata = tst, type = "prob")[, 1L], 
  response = tst$Edibility,
  levels = rev(levels(tst$Edibility))
)
plot(roc_trn)
...
```{r mushroom-cart-roc-02, echo=FALSE, fig.width=5, fig.height=5, out.width="80%"}
# Load required packages
library(pROC)

# Plot train and test ROC curves
roc_trn <- roc(  # train AUC: 0.9852
  predictor = predict(cart1, newdata = trn, type = "prob")[, 1L], 
  response = trn$Edibility,
  levels = rev(levels(trn$Edibility))
)
roc_tst <- roc(  # test AUC: 0.9842
  predictor = predict(cart1, newdata = tst, type = "prob")[, 1L], 
  response = tst$Edibility,
  levels = rev(levels(tst$Edibility))
)
...
knitr::include_graphics("images/this-is-spam.jpg")
```
...
knitr::include_graphics("images/esl-tbl.png")
```
<br>
...
knitr::include_graphics("images/millionaire.png")
```
...
.large[Remember this data set?]

```{r circle-01, echo=FALSE, fig.width=4, fig.height=4, out.width="70%"}
# Simulate data
set.seed(1432)
...
```{r circle-02, echo=FALSE, fig.width=8, fig.height=4, out.width="100%"}
...
knitr::include_graphics("images/bells-and-whistles.jpg")
```
...
knitr::include_graphics("images/pdp-logo-trees.png")
```
...
p1 <- pdp::partial(boston_ranger, pred.var = "lstat", plot = TRUE)
p2 <- pdp::partial(boston_ranger, pred.var = "rm", plot = TRUE)
p3 <- pdp::partial(boston_ranger, pred.var = c("lstat", "rm"),  #<<
...
p3 <- boston_ranger %>%  # %>% is automatically imported!
  pdp::partial(pred.var = "rm", ice = TRUE, center = TRUE) %>%
  autoplot(alpha = 0.1)
```
...
Fit a random forest to the training data from the job attrition example. What is the OOB error for each class? What is the corresponding test error? Which variables seem to be the most important? Does this match with the SVM results obtained earlier?
...
attrition <- attrition %>%
  dplyr::mutate(  # convert some numeric features to factors
...
# Fit a random forest
set.seed(1710)
attr_rf <- randomForest(
  x = subset(trn, select = -Attrition),
  y = trn$Attrition,
  xtest = subset(tst, select = -Attrition),             #<<
  ytest = tst$Attrition,                                #<<
  ntree = 1000,
  strata = trn$Attrition,                               #<<
  sampsize = rep(min(table(trn$Attrition)), times = 2)  #<<
)
attr_rf  # test error ~ 16.36%
...
knitr::include_graphics("images/gpu.jpg")
```
...
```{r ames-xgboost-cv, echo=TRUE, out.width="100%"}
# Load required packages
library(xgboost)
...
```{r ames-xgboost-cv-plot, echo=FALSE, fig.width=5, fig.height=5, out.width="80%"}
...
# Partial dependence plots
oq_ice <- pdp::partial(ames_xgb, pred.var = "Overall_Qual", ice = TRUE, 
                       center = TRUE, train = X)
p4 <- autoplot(pdp::partial(ames_xgb, pred.var = "Gr_Liv_Area", train = X))
p5 <- autoplot(pdp::partial(ames_xgb, pred.var = "Garage_Cars", train = X))
...
pds <- lapply(feats, FUN = function(x) {
  pd <- cbind(x, pdp::partial(ames_xgb, pred.var = x, train = X))
...
knitr::include_graphics("images/questions-trees.png")
```
